...
const Home_Service_Card = ({ service }: {service: IServiceDataType}) => {
    ...
    return (
        <div className="relative rounded-[15px] overflow-hidden min-h-[450px] cursor-pointer" onClick={()=>navigate(`/services/${service.id}`)} >
            ...
            <div className="w-full h-full duration-300 bg-[rgba(59,92,119,0.50)] group-hover:bg-[rgba(59,92,119,0.85)]  flex flex-col items-center justify-center text-center">
                ...
                <h1 className="text-white font-bold text-2xl mb-5">{service.title}</h1>
                <h3 className="text-white font-medium">{service.sub_title}</h3>
            </div>
            </div>
        </div>
    );
